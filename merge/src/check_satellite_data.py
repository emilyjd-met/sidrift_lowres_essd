'''Check number of valid ice pixels per day and plot graph of counts, for purpose of deciding a threshold of when a satellite image has enough valid data'''
...
def check_satellite_data(startdate, enddate, duration, indir, outfile,
    ...
    with open(outfile, 'a+') as outf:
        ...
        while ddate < edate:
            ...
            countgrid = np.zeros((ny, nx), dtype=np.int8)
            ...
            for infile in infiles:
                try:
                    ...
                    pass
            count = np.sum(countgrid)
            countlist.append(count)
            istr = ','.join(ilist)
            outf.write("{}\t{}\t{}\t{}\n".format(ddate, count, fcount, istr))

            ddatesl.append(ddate)
            countsl.append(count)
            fcountsl.append(fcount)
            istrsl.append(istr)

    ddates  = [x for y, x in sorted(zip(countsl, ddatesl))]
    fcounts  = [x for y, x in sorted(zip(countsl, fcountsl))]
    istrs  = [x for y, x in sorted(zip(countsl, istrsl))]
    counts = sorted(countsl)
    ...
    with open(sortfile, 'a+') as sortf:
        for i in range(len(counts)):
            sortf.write("{}\t{}\t{}\t{}\n".format(ddates[i], counts[i],
                                                  fcounts[i], istrs[i]))


    # Plotting a histogram
    fig = plt.figure(figsize=(8,6))
    plt.hist(countlist, density=True, bins=30)
